...
int main(int argc, char** argv) {
    ...
    // Output Data
    if (result==-1) cout<<"Name not found"<<endl; else { cout<<"Name found, "<<endl;
    
    return 0;
}

int search(const int array[], int size, int name) {
    int first = 0,
            last = size - 1,
            middle,
            position = -1;
    ...
    while(!found && first <= last){
        middle = (first+last)/2;
        if (array[middle]==name) found = true, position = middle;
    }  if (array[middle] > name) last = middle-1;
    else first = middle+1;
